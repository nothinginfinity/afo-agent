...
function cairnStonesHtml(env) {
  ...
  return html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AFO BYOK Stones</title><style>body{margin:0;background:#070b12;color:#eaf1ff;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}main{max-width:1120px;margin:0 auto;padding:18px 12px}h1{font-size:30px;margin:0 0 8px}.muted{color:#9fb2d5;line-height:1.45}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media(max-width:860px){.grid{grid-template-columns:1fr}}.card,.stone{background:#0d1525;border:1px solid #253a5d;border-radius:18px;padding:14px;margin:10px 0}label{display:block;margin:10px 0 5px;color:#c1d1ed;font-size:13px;font-weight:700}select,input,textarea,button{width:100%;box-sizing:border-box;background:#07101e;color:#f6f9ff;border:1px solid #30486d;border-radius:11px;padding:10px;font:inherit}textarea{min-height:104px}button{margin-top:10px;border:0;background:#377dff;font-weight:800;cursor:pointer;touch-action:manipulation}.ok{background:#56e3ad;color:#06130c}.warn{background:#3b1019;border-color:#8a3143;color:#ffd9df}.links{display:flex;gap:10px;flex-wrap:wrap;margin:12px 0 16px}a{color:#9cc7ff}.pill{display:inline-flex;border:1px solid #2d466d;border-radius:999px;padding:3px 8px;margin:2px;color:#bfd2ef}.mono,pre{font-family:ui-monospace,Menlo,monospace;font-size:12px;white-space:pre-wrap;word-break:break-word}.text{white-space:pre-wrap;line-height:1.55}</style></head><body><main><h1>AFO BYOK Stones</h1><p class="muted">Safe-mode CairnStone workbench. This version avoids inline tap handlers and prints every error on screen.</p><div class="links"><a href="/">BYOK Chat</a><a id="healthLink" target="_blank">Cairn health</a><a id="jsonLink" target="_blank">Stones JSON</a></div><div class="grid"><section class="card"><h2>Stone Vault</h2><input id="q" placeholder="Search stones"><button id="loadBtn" type="button" class="ok">Load / Search Stones</button><button id="copyBtn" type="button">Copy selected hashes</button><label>Manual stone hashes</label><textarea id="manualHashes">ae60e6833ae2fc7121a2fabf331938df57c67c26dff59bfd733ff47ba71e0402</textarea><button id="manualBtn" type="button">Use manual hashes</button><div id="stats" class="muted">Ready.</div><pre id="debug" class="muted"></pre><div id="stones"></div></section><section class="card"><h2>Stone Brain</h2><label>Provider</label><select id="provider"></select><label>Model</label><select id="preset"></select><input id="model"><div id="baseWrap" style="display:none"><label>Base URL</label><input id="baseUrl" placeholder="https://provider.example/v1"></div><label>BYOK token</label><input id="token" type="password" autocomplete="off"><label>Question</label><textarea id="prompt">Review the selected stones and tell me what to inspect next.</textarea><label>Top K chunks per stone</label><input id="topK" type="number" min="1" max="5" value="1"><button id="askBtn" type="button" class="ok">Attach selected stones to chat</button><button id="queryBtn" type="button">Query selected only</button><pre id="out">Paste/select stones, then test query.</pre></section></div><section class="card"><h2>Create GitHub Stone</h2><div class="grid"><div><label>Owner</label><input id="owner" value="nothinginfinity"></div><div><label>Repo</label><input id="repo" placeholder="contractor-v005-dev"></div><div><label>Path</label><input id="path" placeholder="workers/name/file.js"></div><div><label>Ref</label><input id="ref" value="main"></div><div><label>Title</label><input id="title" placeholder="Repo file stone"></div><div><label>Author</label><input id="author" value="jared"></div></div><button id="createBtn" type="button" class="ok">Create stone server-side</button><pre id="created"></pre></section></main><script id="boot" type="application/json">${boot}</script><script>;(function(){function el(id){return document.getElementById(id)}function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}function log(msg,obj){var box=el('debug');if(box)box.textContent='['+new Date().toLocaleTimeString()+'] '+msg+(obj?'\n'+JSON.stringify(obj,null,2):'')}function setOut(v){el('out').textContent=typeof v==='string'?v:JSON.stringify(v,null,2)}var BOOT=JSON.parse(el('boot').textContent);var stones=[];var selected=new Set(JSON.parse(localStorage.getItem('cairn_selected_stones')||'[]'));function save(){localStorage.setItem('cairn_selected_stones',JSON.stringify(Array.from(selected)))}function manualHashes(){return String(el('manualHashes').value||'').split(/[\s,]+/).map(function(x){return x.trim()}).filter(function(x){return x.length>20})}function allHashes(){var out=new Set(Array.from(selected));manualHashes().forEach(function(h){out.add(h)});return Array.from(out)}function renderStatus(){el('stats').textContent=stones.length+' stones loaded · '+allHashes().length+' selected/manual'}function renderStones(){el('stones').innerHTML=stones.map(function(s){var checked=selected.has(s.hash)?'checked':'';return '<div class="stone"><label><input class="stoneCheck" type="checkbox" data-hash="'+esc(s.hash)+'" '+checked+'> <b>'+esc(s.title||s.hash)+'</b></label><div class="muted">'+esc((s.repo||'')+' '+(s.path||''))+'</div><div class="mono">'+esc(s.hash)+'</div><span class="pill">refs '+esc(s.refs_count)+'</span><span class="pill">ratio '+esc(s.ratio)+'x</span><span class="pill">raw '+Number(s.original_bytes||0).toLocaleString()+'</span><div class="muted">'+esc(s.lod4||s.lod5||'')+'</div></div>'}).join('')||'<div class="stone muted">No stones loaded yet. Manual hash is available above.</div>';Array.from(document.querySelectorAll('.stoneCheck')).forEach(function(box){box.addEventListener('change',function(){if(box.checked)selected.add(box.dataset.hash);else selected.delete(box.dataset.hash);save();renderStatus()})});renderStatus()}async function fetchJson(url,options,ms){ms=ms||12000;var ctrl=new AbortController();var timer=setTimeout(function(){try{ctrl.abort()}catch(e){}},ms);try{var r=await fetch(url,Object.assign({},options||{},{cache:'no-store',signal:ctrl.signal}));var text=await r.text();var d;try{d=JSON.parse(text)}catch(e){d={ok:false,error:'non_json_response',status:r.status,preview:text.slice(0,1000)}}d.http_status=r.status;if(!r.ok&&d.ok!==false)d.ok=false;return d}finally{clearTimeout(timer)}}async function loadStones(){el('stats').textContent='Loading stones...';log('tap: load stones',{url:BOOT.cairnstoneUrl+'/v1/stones'});var d=await fetchJson(BOOT.cairnstoneUrl+'/v1/stones?q='+encodeURIComponent(el('q').value)+'&t='+Date.now());if(!d.ok){log('direct failed, trying proxy',d);d=await fetchJson('/api/cairn/stones?q='+encodeURIComponent(el('q').value)+'&t='+Date.now())}if(!d.ok){el('stats').textContent='Stone list failed. Manual hash still works.';el('stones').innerHTML='<div class="card warn">'+esc(JSON.stringify(d,null,2))+'</div>';return}stones=d.stones||[];log('loaded stones',{total:d.total,stones:stones.length});renderStones()}function useManual(){manualHashes().forEach(function(h){selected.add(h)});save();renderStatus();setOut('Manual hashes ready:\n'+allHashes().join('\n'));log('manual hashes selected',{hashes:allHashes()})}async function retrieve(){var hashes=allHashes();if(!hashes.length)throw new Error('Select a stone or paste a stone hash first.');var q=el('prompt').value||'summary';var out=[];for(var i=0;i<hashes.length;i++){log('query stone',{hash:hashes[i],query:q});var d=await fetchJson(BOOT.cairnstoneUrl+'/v1/query-expand',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({stone_hash:hashes[i],query:q,top_k:Number(el('topK').value||1),context_lines:0,include_metadata:true})});out.push({hash:hashes[i],result:d})}return out}async function queryOnly(){try{setOut('Querying selected/manual stones...');setOut(await retrieve())}catch(e){setOut(e.message||String(e));log('query error',{message:e.message||String(e)})}}async function ask(){try{setOut('Retrieving stone context...');var contexts=await retrieve();var contextText=contexts.map(function(c,i){return 'STONE '+(i+1)+' '+c.hash+'\n'+(c.result.text||'No matching context.')}).join('\n\n====\n\n');var prompt='Use the retrieved CairnStone context below. Cite stone hashes when useful.\n\n'+contextText+'\n\nUser question:\n'+el('prompt').value;setOut('Calling BYOK chat...');var d=await fetchJson('/api/byok/chat',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({provider:el('provider').value,model:el('model').value,baseUrl:el('baseUrl').value,providerKey:el('token').value,prompt:prompt,maxTokens:1200})},30000);el('out').innerHTML=d.ok!==false?'<div class="text">'+esc(d.text||'')+'</div><pre>'+esc(JSON.stringify(d,null,2))+'</pre>':'<pre>'+esc(JSON.stringify(d,null,2))+'</pre>'}catch(e){setOut(e.message||String(e));log('stone brain error',{message:e.message||String(e)})}}async function createStone(){try{el('created').textContent='Creating...';var body={owner:el('owner').value,repo:el('repo').value,path:el('path').value,ref:el('ref').value,title:el('title').value,author:el('author').value,metadata:{created_from:'afo_byok_stones_safe'}};var d=await fetchJson(BOOT.cairnstoneUrl+'/v1/stones/github',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)},30000);el('created').textContent=JSON.stringify(d,null,2);if(d.stone_hash){selected.add(d.stone_hash);save();await loadStones()}}catch(e){el('created').textContent=e.message||String(e);log('create error',{message:e.message||String(e)})}}function fillProviders(){el('provider').innerHTML=BOOT.providers.map(function(p){return '<option value="'+esc(p)+'">'+esc(BOOT.labels[p]||p)+'</option>'}).join('');fillModels()}function fillModels(){var p=el('provider').value;var ms=BOOT.modelOptions[p]||['custom-model'];el('preset').innerHTML=ms.map(function(m){return '<option value="'+esc(m)+'">'+esc(m)+'</option>'}).join('')+'<option value="custom">Custom</option>';el('model').value=ms[0]||'';el('baseWrap').style.display=p==='openai-compatible'?'block':'none'}function copySelected(){navigator.clipboard.writeText(allHashes().join('\n')).then(function(){log('copied hashes',{count:allHashes().length})}).catch(function(e){log('copy failed',{message:e.message||String(e)})})}try{el('healthLink').href=BOOT.cairnstoneUrl+'/health';el('jsonLink').href=BOOT.cairnstoneUrl+'/v1/stones';fillProviders();el('provider').addEventListener('change',fillModels);el('preset').addEventListener('change',function(){if(el('preset').value!=='custom')el('model').value=el('preset').value});el('loadBtn').addEventListener('click',loadStones);el('manualBtn').addEventListener('click',useManual);el('queryBtn').addEventListener('click',queryOnly);el('askBtn').addEventListener('click',ask);el('createBtn').addEventListener('click',createStone);el('copyBtn').addEventListener('click',copySelected);renderStones();log('safe-mode script ready',{buildId:BOOT.buildId,cairnstoneUrl:BOOT.cairnstoneUrl})}catch(e){document.body.insertAdjacentHTML('afterbegin','<pre style="background:#3b1019;color:#ffd9df;padding:12px">SCRIPT BOOT ERROR: '+esc(e.message||String(e))+'</pre>')}})();</script></body></html>`);
}
