...
function cairnStonesHtml(env) {
  ...
  return html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AFO BYOK Stones</title><style>body{margin:0;background:#070b12;color:#eaf1ff;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}main{max-width:1120px;margin:0 auto;padding:18px 12px}h1{font-size:30px;margin:0 0 8px}.muted{color:#9fb2d5;line-height:1.45}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media(max-width:860px){.grid{grid-template-columns:1fr}}.card,.stone{background:#0d1525;border:1px solid #253a5d;border-radius:18px;padding:14px;margin:10px 0}label{display:block;margin:10px 0 5px;color:#c1d1ed;font-size:13px;font-weight:700}select,input,textarea,button{width:100%;box-sizing:border-box;background:#07101e;color:#f6f9ff;border:1px solid #30486d;border-radius:11px;padding:10px;font:inherit}textarea{min-height:120px}button{margin-top:10px;border:0;background:#377dff;font-weight:800;cursor:pointer}.ok{background:#56e3ad;color:#06130c}.warn{background:#3b1019;border-color:#8a3143;color:#ffd9df}.links{display:flex;gap:10px;flex-wrap:wrap;margin:12px 0 16px}a{color:#9cc7ff}.pill{display:inline-flex;border:1px solid #2d466d;border-radius:999px;padding:3px 8px;margin:2px;color:#bfd2ef}.mono,pre{font-family:ui-monospace,Menlo,monospace;font-size:12px;white-space:pre-wrap;word-break:break-word}.text{white-space:pre-wrap;line-height:1.55}</style></head><body><main><h1>🪨 AFO BYOK Stones</h1><p class="muted">Proxy-mode CairnStone workbench inside your existing BYOK app. Create GitHub file stones, select stones, retrieve only matching chunks, then attach the retrieved context to BYOK chat.</p><div class="links"><a href="/">BYOK Chat</a><a href="/health">Health</a><a href="${cairnstoneBase(env)}/health" target="_blank">Cairn health</a><a href="${cairnstoneBase(env)}/v1/stones" target="_blank">Stones JSON</a></div><div class="grid"><section class="card"><h2>Stone Vault</h2><input id="q" placeholder="Search stones"><button class="ok" onclick="loadStones()">Load / Search Stones</button><button onclick="copySelected()">Copy selected hashes</button><div id="stats" class="muted">Loading...</div><div id="stones"></div></section><section class="card"><h2>Stone Brain</h2><label>Provider</label><select id="provider" onchange="fillModels()"></select><label>Model</label><select id="preset" onchange="syncModel()"></select><input id="model"><div id="baseWrap" style="display:none"><label>Base URL</label><input id="baseUrl" placeholder="https://provider.example/v1"></div><label>BYOK token</label><input id="token" type="password" autocomplete="off"><label>Question</label><textarea id="prompt">Review the selected stones and tell me what to inspect next.</textarea><label>Top K chunks per stone</label><input id="topK" type="number" min="1" max="5" value="1"><button class="ok" onclick="askStoneBrain()">Attach selected stones to chat</button><button onclick="queryOnly()">Query selected only</button><pre id="out">Select stones, then ask.</pre></section></div><section class="card"><h2>Create GitHub Stone</h2><div class="grid"><div><label>Owner</label><input id="owner" value="nothinginfinity"></div><div><label>Repo</label><input id="repo" placeholder="contractor-v005-dev"></div><div><label>Path</label><input id="path" placeholder="workers/name/file.js"></div><div><label>Ref</label><input id="ref" value="main"></div><div><label>Title</label><input id="title" placeholder="Repo file stone"></div><div><label>Author</label><input id="author" value="jared"></div></div><button class="ok" onclick="createStone()">Create stone server-side</button><pre id="created"></pre></section></main><script id="boot" type="application/json">${boot}</script><script>const BOOT=JSON.parse(document.getElementById('boot').textContent);const E=id=>document.getElementById(id);const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let stones=[];let selected=new Set(JSON.parse(localStorage.getItem('cairn_selected_stones')||'[]'));function saveSel(){localStorage.setItem('cairn_selected_stones',JSON.stringify([...selected]))}function fillProviders(){E('provider').innerHTML=BOOT.providers.map(p=>'<option value="'+esc(p)+'">'+esc(BOOT.labels[p]||p)+'</option>').join('');fillModels()}function fillModels(){const p=E('provider').value;const ms=BOOT.modelOptions[p]||['custom-model'];E('preset').innerHTML=ms.map(m=>'<option value="'+esc(m)+'">'+esc(m)+'</option>').join('')+'<option value="custom">Custom</option>';E('model').value=ms[0]||'';E('baseWrap').style.display=p==='openai-compatible'?'block':'none'}function syncModel(){if(E('preset').value!=='custom')E('model').value=E('preset').value}function render(){E('stones').innerHTML=stones.map(s=>'<div class="stone"><label><input type="checkbox" '+(selected.has(s.hash)?'checked':'')+' onchange="this.checked?selected.add(\''+s.hash+'\'):selected.delete(\''+s.hash+'\');saveSel();renderShare()"> <b>'+esc(s.title||s.hash)+'</b></label><div class="muted">'+esc(s.repo||'')+' '+esc(s.path||'')+'</div><div class="mono">'+esc(s.hash)+'</div><span class="pill">refs '+esc(s.refs_count)+'</span><span class="pill">ratio '+esc(s.ratio)+'x</span><span class="pill">raw '+Number(s.original_bytes||0).toLocaleString()+'</span><div class="muted">'+esc(s.lod4||s.lod5||'')+'</div></div>').join('')||'<div class="stone muted">No stones found.</div>';renderShare()}function renderShare(){E('stats').textContent=stones.length+' stones loaded · '+selected.size+' selected'}async function loadStones(){E('stats').textContent='Loading stones...';const r=await fetch(BOOT.cairnstoneUrl+'/v1/stones?q='+encodeURIComponent(E('q').value),{cache:'no-store'});const d=await r.json();if(!d.ok){E('stones').innerHTML='<div class="card warn">'+esc(JSON.stringify(d,null,2))+'</div>';return}stones=d.stones||[];render()}async function createStone(){E('created').textContent='Creating...';const body={owner:E('owner').value,repo:E('repo').value,path:E('path').value,ref:E('ref').value,title:E('title').value,author:E('author').value,metadata:{created_from:'afo_byok_stones'}};const d=await (await fetch(BOOT.cairnstoneUrl+'/v1/stones/github',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)})).json();E('created').textContent=JSON.stringify(d,null,2);if(d.stone_hash){selected.add(d.stone_hash);saveSel();await loadStones()}}async function retrieveContexts(){const q=E('prompt').value||'summary';const out=[];for(const hash of selected){const d=await (await fetch(BOOT.cairnstoneUrl+'/v1/query-expand',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({stone_hash:hash,query:q,top_k:Number(E('topK').value||1),context_lines:0,include_metadata:true})})).json();out.push({hash,result:d})}return out}async function queryOnly(){E('out').textContent='Querying selected stones...';E('out').textContent=JSON.stringify(await retrieveContexts(),null,2)}async function askStoneBrain(){E('out').textContent='Retrieving stone context...';const contexts=await retrieveContexts();const contextText=contexts.map((c,i)=>'STONE '+(i+1)+' '+c.hash+'\\n'+(c.result.text||'No matching context.')).join('\\n\\n====\\n\\n');const prompt='Use the retrieved CairnStone context below. Cite stone hashes when useful.\\n\\n'+contextText+'\\n\\nUser question:\\n'+E('prompt').value;E('out').textContent='Calling BYOK chat...';const r=await fetch('/api/byok/chat',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({provider:E('provider').value,model:E('model').value,baseUrl:E('baseUrl').value,providerKey:E('token').value,prompt,maxTokens:1200})});const d=await r.json();E('out').innerHTML=d.ok!==false?'<div class="text">'+esc(d.text||'')+'</div><pre>'+esc(JSON.stringify(d,null,2))+'</pre>':'<pre>'+esc(JSON.stringify(d,null,2))+'</pre>'}async function copySelected(){await navigator.clipboard.writeText([...selected].join('\\n'));alert('copied')}fillProviders();loadStones().catch(e=>{E('stones').innerHTML='<div class="card warn">'+esc(e.message||String(e))+'</div>'})</script></body></html>`);
}
